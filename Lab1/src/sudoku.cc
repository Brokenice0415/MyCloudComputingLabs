...
Column* Dance::new_column(int n = 0)
{
...
void Dance::append_column(int n)
{
...
int Dance::get_row_col(int row, int val)
{
    return kRow+row*10+val;
}

int Dance::get_col_col(int col, int val)
{
    return kCol+col*10+val;
}

int Dance::get_box_col(int box, int val)
{
    return kBox+box*10+val;
}

Dance::Dance(int inout[81]) : inout_(inout), cur_node_(0)
{
    stack_.reserve(100);
...
void Dance::put_left(Column* old, Column* nnew)
{
    nnew->left = old->left;
    nnew->right = old;
    old->left->right = nnew;
...
void Dance::put_up(Column* old, Node* nnew)
{
    nnew->up = old->up;
    nnew->down = old;
    old->up->down = nnew;
